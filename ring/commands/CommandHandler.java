...
public final class CommandHandler {
	...
	//boolean value and a String. The boolean indicates if the command "failed." The String is what gets
	...
	public CommandHandler(CommandSender sender) {
		...
		for (int x = 0; x < m.length; x++) {
			...
			//Stores the Method object into a tree with the name "CMD_cmdhere" as key.
	...
	//sendCommand method.
	//This method splits a command up and calls handleCommand(). It returns a CommandResult that the
	//sender can use.
	...
	//for ambiguous fragments. If no suitable command can be found, it returns null.
	private boolean completeCommand(Command comm) {
		//set up some initial information, like a list of all available commands.
		String fragment = "CMD_" + comm.getActualCommand();

		//if the fragment is only 1 letter, it's not something we should bother looking
		//up; there is TOO much ambiguity. 1 letter commands are registered as alternate commands.
	...
	private synchronized CommandResult handleCommand(Command cmd) {
		...
		//Get the list of declared methods... will probably more efficient later by creating it on
		//instantiation.
		m = (Method)commands.get(methodName);

		//If m is null it obviously doesn't exist so.... EXECUTE THE BAD COMMAND METHOD! :D
		if (m == null) {
			//The line below sets the command sent itself to "bad" in order to get rid of the parameters
		...
		try {
			...
			if (nMethodParams == 0) {System.out.println("no params"); res = (CommandResult)m.invoke(this); }

			//commands with parameters. however, they may still not need parameters to operate.
			else {
				System.out.println("params");
				if (cmd.getParameterLength() == 0) { System.out.println("execing null param"); res = (CommandResult) m.invoke(this, parameters); }
				else res = (CommandResult) m.invoke(this, new Object[] { parameters });
			}
		}
			...
			System.err.println("InvocationTargetException for CMD_" + cmd.getActualCommand() + "(). This most likely means there is a NullPointerException issue in the command code. It may also be a ClassCastException issue.");
			e.printStackTrace();
		}
		//Fix that little bug with the no-parameter methods like the directional ones.
			...
			/*
      try {
        res = (CommandResult)m.invoke(this, (Object[])null);
      }
      catch (IllegalAccessException ex) {
        System.out.println("UHOH11");
      }
      catch (InvocationTargetException ex) {
        System.err.println("InvocationTargetException for CMD_" + cmd.getActualCommand() + "(). This most likely means there is a Null Pointer issue in the command code.");
      }
			 */
		}
	...
	//This method checks the current command to see if it is an alternate command.
	//Manipulates the command array directly. Returns true if successful, false if not.
	private boolean checkAlternateCommand(Command cmd) {
		...
		if (m == null) {
			String realCmd = (String)alternateCommands.get(cmd.getActualCommand());
			if (!(cmd.getActualCommand().equals(realCmd)) && realCmd != null) { cmd.setActualCommand(realCmd); return true; }
		}
	...
	//This method registers an alternate--often shortened--command that is linked to another command.
	//This is used for things like "north" and "n" being the same in-game. This is a crude system and
	//I plan to implement an auto-text algorithm with priorities built in later.
	//
	...
	//All commands have parameters in the method signature that match the MAXIMUM possible parameters
	//they can have in-game. Since we are not using JDK 1.5, a limitation of this system is that we
	//cannot have commands with infinite parameters without a complicated implementation, although
	//I doubt we shall actually need that.

	//If the parameter is not used in the in-game command, the call to the method should simply
	//send a null parameter and the code will handle it accordingly.

	//Bad command.
	//This isn't really a command. It's just a method to execute in order to return an error message
	//of sorts when a player types in something wrong so the MUD won't blow up. Though theoretically someone
	...
	private CommandResult CMD_look(CommandParameters params) {
		...
		//First check to see if we are just looking at the room. ie: sending just the "look" command.
		if (t == null) {
			Room room = mob.getLocation().getRoom();

			if (mob.isBlind)  {
				res.setFailText("You see nothing, for you are blind!");
				World.sendVisualToLocation(mob, mob.getName() + " stumbles about blindly.", null);
				return res;
			}
		...
		//Now check to see if we are looking at something specific, if true, return the look description
		//of the thing being looked at it.
		else {
			if ((!(t instanceof Affectable)) && (t != null)) return res;
	...
	//Say command.
	//This command says something from the sender to all other mobiles in the room.
	//This method is a special case and will not get called by the handleCommand method.
	private CommandResult CMD_say(CommandParameters params) {
		...
		if (mob.isDeaf) textBackToPlayer = "You say something, but you're not entirely sure what since you cannot hear!";
		else textBackToPlayer = "You say, \"";
		...
		World.sendAudioToLocation(mob, textToOtherPlayers, mob.getName() + " says something, but you cannot hear it!\n");
	...
	private CommandResult CMD_south() {
		//Make the CommandResult object.
		CommandResult res = new CommandResult();
		res.setFailText("[GREEN]You cannot go that way.[WHITE]");
		Mobile mob = (Mobile)sender;
	...
	//Score command.
	//This shows the score of the mobile enacting it... Which is usually a player. What the heck would
	...
	private CommandResult CMD_score() {
		...
		Mobile mob = (Mobile)sender;
		String score = "\n[R][WHITE]Score and Statistics for [B][GREEN]" + mob.getName() +
		"[R][WHITE]" + "\nAlignment: " + mob.getAlignment().getAlignmentString() +
		"\nHP: [B][RED]" + mob.getCurrentHPString() + "/" + mob.getMaxHPString()
		+ " [R][WHITE]Class: " + mob.getMobileClass().getDisplayName() +
		" [R][WHITE]Level: [B][MAGENTA]" + mob.getLevel() + "[R][WHITE] Class Type: "
	...
	private CommandResult CMD_get(CommandParameters params) {
		...
		Room room = getter.getLocation().getRoom();
		...
		World.sendVisualToLocation(getter, getter.getName() + " picks up " +i.getIndefiniteDescriptor().toLowerCase() + " " + i.getName() + "[R][WHITE].", null);
	...
	private CommandResult CMD_drop(CommandParameters params) {
		...
		//Set the text.
		res.setText("[R][WHITE]You drop " + item.getIndefiniteDescriptor().toLowerCase() + " " +
				item.getName() + "[R][WHITE].");

		//Notify other people in the world.
		World.sendVisualToLocation(mob, mob.getName() + " drops " + item.getIndefiniteDescriptor().toLowerCase() + " " +
	...
	private CommandResult CMD_inventory() {
		...
		if ((inventory == null) || (inventory.size() == 0)) return res;

		for (int c = 0; c < inventory.size(); c++) {
			text += ((Item)inventory.get(c)).getName() + "\n";
		}
	...
	private CommandResult CMD_cast(CommandParameters params) {
		...
		//Due to the way the new Spell system works, this command does not take a Spell as a
		//parameter. It will have to take a String as the name of the spell and look it up via
		...
		//Check the target type of the spell. If is "ROOM" then leave the "target"
		...
		//Loop through each target/thing affected by the spell and check spell resistance and such.
		//In this loop: Each target gets their saving throw and the like.
		//In this loop: APPLY THE EFFECTS OF THE SPELL TO THE TARGET IF THEY ARE AFFECTED.
		//End the loop.
		//Finish this method.
	...
	private CommandResult CMD_wear(CommandParameters params) {
		...
		if (t == null) {res.setFailText("[R][WHITE]Wear what?"); return res;}
		if ((!(t instanceof Item)) && (t != null)) return res;

		Item target = (Item)t;
		if (!target.wearable()) return res;

		//We've checked everything dealing with if the thing is wearable or not.
		Mobile mob = (Mobile)sender;
		...
		//Check if the wearer meets all the requirements to wear target (alignment, class, etc.)


		//Check if wearer has the actual body part to wear the target on. The method that will do
		//this actually returns an integer corresponding to the number of eligible body parts
		//on the wearer's body. If this number is greater than one, the next step has a special
		//addition.
		BodyPart[] eligibleParts = mobBody.getEligibleBodyParts(target);

		//Were there any parts actually found?
		if (eligibleParts == null) return res;
		System.out.println("Passed step 2...");

		//Check if the wearer has room on the body part. If the number from the previous step was
		//greater than 1, start a for loop equal to the amount of body parts remaining (# of them - 1).
		//Try to equip on each of those body parts.
		boolean success = false;
		BodyPart thePart = new BodyPart(null);
		...
		//Set text and stuff.
		res.setText("[R][WHITE]You wear " + target.getIndefiniteDescriptor().toLowerCase() + " " +
				target.getName() + " on your " + thePart.getName().toLowerCase() + ".");
		res.setSuccessful(true);

		//Notify other players.
		World.sendVisualToLocation(mob, "[R][WHITE]" + mob.getName() + " wears " + target.getIndefiniteDescriptor().toLowerCase() + " " +
				target.getName() + " on your " + thePart.getName().toLowerCase() + ".", null);
	...
	private CommandResult CMD_remove(CommandParameters params) {
		...
		res.setText("[R][WHITE]You remove " + item.getIndefiniteDescriptor().toLowerCase() + " " +
	...
	private CommandResult CMD_search(CommandParameters params) {
		params.init(Command.TXT);
		String dir = (String)params.getParameter(0); //the direction to search in
		...
		ZoneCoordinate loc = mob.getLocation();
		Room room = loc.getRoom();

		//The search command behaves differently for mobiles who are blind.
		//Since the look command returns no useful information for them, they can
		//use the search command to help find their way around the world, although in a
		//very poor fashion. A -4 penalty is applied to the check as per the d20 rules
		//on blindness. The search check total reveals more information as it goes higher:
		//10: 2 random exits.
		//15: 4 random exits.
		//20: 6 random exits. (maximum)
		//For every 1 above 25, one random mobile or entity is unveiled. Their descriptions should be vague.

		//Blind search version
		if (mob.isBlind) {
			check -= 4; //apply -4 penalty
			if (check < 10) {
				res.setFailText("[R][WHITE]You're just as lost as you were when you began this search.");
				return res;
			}

			res.setText("[R][WHITE]You cautiously begin to move around the room, trying to get a sense of its layout through your non-visual senses. (check: " + check + ")");

			//exits first: CURRENTLY THIS IS NOT IMPLEMENTED
			String searchText = "After feeling your way around the room, you [B]THINK[R] you find the following:\nExits: "; //+ room.generateBlindExitsString(check);

			//now mobiles/entities
			searchText += "\nOccupants:\n" + room.generateBlindRandomOccupantList(check);

			mob.setLockMessage("[R][WHITE]You are still searching the room (blindly, no less)!");
		...
		//Regular search version
			...
			res.setText("[R][WHITE]You begin [B]searching[R] the " + dir + " part of the room (check: " + check + ")");
			...
			Room checkRoom = loc.offset3D(ZoneCoordinate.getDirection(dir)).getRoom();
			//first, look for hidden exits and set the mobile's current hidden exit search check to the result of the
			//search check.
			...
			//third, if player has the Trapfinding class feature, look for DC 20+ traps.
			searchText += "Difficult Traps: IMPLEMENT LATER!";

			//now, we need to calculate a lock time based on the square footage of the room.
			//this calculation is currently based on the fact that a 5x5 (25 square feet) area takes 6 seconds (3 ticks)
	...
	private CommandResult CMD_hide() {
		...
		//This falls in the +2/-2 rule for the d20 system (i.e. favorable conditions grant up to a +2 bonus)
		//It's not an official rule, but we're using it here.
		if (mob.isSitting) check += 1;
		if (mob.isProne) check += 2;
		if (mob.isLyingDown) check += 2;

		//This mobile is now hiding. Hidden mobiles do not show up on the list of room occupants
		//and they do not broadcast SPECIFIC (i.e. show their name) messages when leaving or arriving 
		//into a room. However, other mobiles can defeat this with a spot check that is higher than this 
		//mobile's hide check. A successful spot check will show the mobile on the room occupant list as well
		//as show his leaving/arrival messages in rooms. All of this is handled in the direction command 
		//methods (north, south, etc) and the look command. However, spotting and listening work like search: 
		//there are commands for each and only then will a mobile possibly be able to see/hear a hiding mobile.
		//
		//In addition, see CMD_movesilently(). This allows a mobile to move silently. If the mob is
		//NOT moving silently but is hiding, vague room leave/arrive messages will be displayed.
		//Move silently removes the broadcasts.

		mob.hideCheck = check;
		mob.increaseMovementMultiplier(1); //the mobile should move a bit "slower" when hiding.
		res.setText("[R][WHITE]You are now [B][YELLOW]hiding[R][WHITE]! (check: " + check + ")");
		res.setSuccessful(true);
	...
	private CommandResult CMD_movesilently() {
		CommandResult res = new CommandResult();
		res.setFailText("[R][WHITE]You don't seem to be one for sneaking around silently...");
		...
		if (mob.isFighting) {
			res.setFailText("[B][YELLOW]Moving around silently won't help in combat![R][WHITE]");
			return res;
		}

		int check = moveSilently.makeCheck();

		//It will be necessary to implement penalties based on the terrain the mobile is in.
		//That is later though. The d20 SRD specifies the following penalties for terrain:
		//Noisy (scree, shallow or deep bog, undergrowth, dense rubble): -2 penalty
		//Very noisy (dense undergrowth, deep snow): -5 penalty
		
		//This mobile is now moving silently. Mobiles sneaking about do not broadcast room leave/arrive
		//messages. However, other mobiles can defeat this with a listen check that is higher than this mobile's
		//move silently check. A successful listen check will cause the mobile to broadcast arrive/leave messages
		//in the room of the listeninng mobile. All of this is handled in the direction command methods 
		//(north, south, etc) and the look command. However, listening works like search: 
		//there is a listen command and it resets every time a mobile goes into a new room. Once the command is
		//activated, only then will a mobile possibly be able to hear a moving silently mobile.
		//
		//In addition, see CMD_hide(). This allows a mobile to hide. If the mob is
		//NOT hiding but is moving silently, they will show up on room occupant lists with a look
		//command. However, there will be broadcasting of arrive/leave messages.

		mob.moveSilentlyCheck = check;
		mob.increaseMovementMultiplier(1); //the mobile should move a bit "slower" when moving silently.
		res.setText("[R][WHITE]You are now [B][YELLOW]moving silently[R][WHITE]! (check: " + check + ")");
		res.setSuccessful(true);
	...
	private CommandResult CMD_listen() {
		...
		Mobile mob = (Mobile)sender;
		//is our listener deaf? if so, he can't really listen for anything can he?
		if (mob.isDeaf) {
	...
	private CommandResult CMD_who(CommandParameters params) {
		...
		//class names, player names, race names, individual level (a number), a level range (#-#),
		//rp, lfg, chaotic, lawful, neutral, good, evil, god, staff, immortal, mortal,
		//forger, greater, lesser, admin, areas, rpgod, code, zone, invis (<-THIS LINE FOR GODS ONLY!!)

		//IF AN ILLEGAL PARAMETER IS FOUND, simply return res and it will return the failed
		//CommandResult with the error message. Illegal parameters are:
		// Anything that's not in the above list.
		// Anything a player should not have access to (i.e. mortals doing "who zone" or "who admin")

		//Produce a String that meets all acceptable parameters.
		//To do this, run through the list of all players and test their whotag against the parameter list string of
		//this command. A player's whotag has the following format: "playername racename level classname zonename ethical moral <god status> <player set status> <admin set status>"
		//For example:
		//"ao human 13 fighter Waterdeep neutral evil forger rp code areas"

		//A simple indexOf test or something similar against all whotags will allow us to construct the list of
	...
	private CommandResult CMD_equipment() {
		...
		for (int c = 0; c < mobEquipment.size(); c++) {
			Item item = (Item)mobEquipment.get(c);
			System.out.println("Got an item...: " + item);
			if (item != null)
				equipment += "<worn on " + item.partWornOn().getName() + "> " + item.getName() + "\n";
		}
	...
	private CommandResult CMD_attributes() {
		...
		String attributes = "[R][WHITE]Character Attributes for [B][GREEN]" + mob.getName() +
		"[R][WHITE]:\nStrength: " + mob.getStat(Mobile.STRENGTH) + " Intelligence: " +
		mob.getStat(Mobile.INTELLIGENCE) + " Wisdom: " + mob.getStat(Mobile.WISDOM) +
		" Dexterity: " + mob.getStat(Mobile.DEXTERITY)
		+ " Charisma: " + mob.getStat(Mobile.CHARISMA) + " Constitution: " +
		mob.getStat(Mobile.CONSTITUTION) + "\n\nSaving Throws: Fort [B][YELLOW]+0[R][WHITE] Ref [B][YELLOW]+0[R][WHITE] Will [[B][YELLOW]+0]"
		+ "\n[R][WHITE]Spell Resistance: [MAGENTA]0[WHITE] Armor Class: [B][WHITE]" + mob.getAC() + "[R][WHITE]";
	...
	private CommandResult CMD_prepare(CommandParameters params) {
		...
		if (spell == null) {
			res.setFailText("[GREEN]You don't have that spell in your spellbook.");
			return res;
		}
		...
		//Are they actually able to prepare the spell? (Not in battle, not incapacitated)?
		//ALONG WITH THIS!!!
	...
	private CommandResult CMD_bash(CommandParameters params) {
		...
		ClassFeature bash = mobClass.getFeatureByName("bash");

		if (bash == null) {
			res.setFailText("[R][WHITE]You don't have that class feature.");
			return res;
		}

		//Target stuff.
		try {
			Affectable ta = (Affectable)t;
			if (ta.getEntityType() == Affectable.NON_LIVING) {
				res.setFailText("[R][WHITE]You can't bash that!");
				return res;
			}
		}
		...
		//If all conditions are met, make the bash attack roll and then if successful, bash the target.
		Affectable target = (Affectable)t;
		boolean hit = mob.attack(target, true);
		if (hit) {
			bash.registerTarget(target);
			bash.activate();
			res.setText("[R][WHITE]" + target.getName() + " is knocked prone by your powerful [B]BASH[R]!");
			res.setSuccessful(true);
		}

		else {
			res.setFailText("[R][WHITE]Your bash misses " + target.getName());
		}
		return res;
	}

	private CommandResult CMD_rage() {
		CommandResult res = new CommandResult();
		res.setFailText("[R][WHITE]You can't go into a rage right now.");

		//Do they even have the class feature?
		Mobile mob = (Mobile)sender;
		MobileClass mobClass = mob.getMobileClass();
		...
		rage.registerTarget(mob);
		rage.registerTimer((mob.getModifier(Mobile.CONSTITUTION) > 0) ? (3 * (mob.getModifier(Mobile.CONSTITUTION) + 2)) : 3);
	...
	private CommandResult CMD_load() {
		...
		try {
			XMLDecoder e = new XMLDecoder(new BufferedInputStream(new FileInputStream(mob.getName() + ".xml")));
	...
	private CommandResult CMD_deaf() {
		CommandResult res = new CommandResult();
		Mobile mob = (Mobile)sender;
		if (mob.isDeaf) mob.isDeaf = false;
		else mob.isDeaf = true;
	...
	private CommandResult CMD_blind() {
		CommandResult res = new CommandResult();
		Mobile mob = (Mobile)sender;
		if (mob.isBlind) mob.isBlind = false;
		else mob.isBlind = true;
